...
export class MatchController {
    ...
    @ApiOperation({
        ...
        description: 'Retrieve all match requests sent by the current user with pagination.'
    })
    ...
    @ApiOperation({
        ...
        description: 'Retrieve all match requests received by the current user (as a listing owner) with pagination.'
    })
    @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 20)' })
    @ApiResponse({
    ...
    @ApiOperation({
        ...
        description: 'Retrieve detailed information about a specific match request.'
    })
    ...
    @ApiResponse({ status: 403, description: 'Forbidden - Not authorized to view this match' })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    async getMatchById(
    ...
    @ApiOperation({
        ...
        description: 'Accept or reject a match request. Only the listing owner can update match status.'
    })
    @ApiParam({ name: 'id', description: 'Match ID', type: 'string' })
    ...
    @ApiResponse({ status: 404, description: 'Match not found' })
    @ApiResponse({ status: 403, description: 'Forbidden - Only listing owner can update status' })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    async updateMatchStatus(
        ...
        const message = updateMatchStatusDto.status === 'ACCEPTED'
