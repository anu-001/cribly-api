...
export class ListingsService {
    ...
    async create(createListingDto: CreateListingDto, ownerId: string) {
        // Validate image URLs
        if (!createListingDto.imageUrls || createListingDto.imageUrls.length === 0) {
    ...
    async findAll(queryDto: QueryListingDto) {
        ...
        return {
            ...
            meta: {
                ...
                page,
                limit,
                totalPages: Math.ceil((radiusKm ? processedListings.length : total) / limit),
            },
        };
    }
    ...
    async update(id: string, updateListingDto: UpdateListingDto, userId: string) {
        ...
        const publishedAt =
            updateListingDto.status === PropertyStatus.ACTIVE && !existingListing.publishedAt
                ? new Date()
                : undefined;
