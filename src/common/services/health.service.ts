...
export class HealthService {
  ...
  async getReadinessStatus(): Promise<{ ready: boolean; checks: any }> {
    ...
    const ready = Object.values(readinessChecks).every(
      (check) => check === true,
    );
  ...
  private async checkServices(): Promise<HealthCheck['services']> {
    ...
    return {
      ...
      cloudinary:
        checks[1].status === 'fulfilled' ? checks[1].value : 'unknown',
      firebase: checks[2].status === 'fulfilled' ? checks[2].value : 'unknown',
    };
  }
