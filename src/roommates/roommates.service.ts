...
export class RoommatesService {
  ...
  async updateProfile(userId: string, updateDto: UpdateRoommateProfileDto) {
    ...
    if (updateDto.budgetMin !== undefined && updateDto.budgetMax !== undefined) {
  ...
  async getCompatibilityScore(userId: string, targetProfileId: string) {
    const [userProfile, targetProfile] = await Promise.all([
      ...
      this.prisma.roommateProfile.findUnique({ where: { id: targetProfileId } }),
    ]);
    ...
    const budgetOverlap = Math.min(userProfile.budgetMax, targetProfile.budgetMax) -
      Math.max(userProfile.budgetMin, targetProfile.budgetMin);
    if (budgetOverlap > 0) {
      const userBudgetRange = userProfile.budgetMax - userProfile.budgetMin;
      const targetBudgetRange = targetProfile.budgetMax - targetProfile.budgetMin;
    ...
    if (commonCities.length > 0) {
      const locationScore = Math.min(100, (commonCities.length / Math.max(userProfile.preferredCities.length, targetProfile.preferredCities.length)) * 100);
    ...
    if (userProfile.smokingPreference === targetProfile.smokingPreference ||
    ...
    // Pet compatibility
    if (userProfile.petPreference === targetProfile.petPreference ||
    ...
    if (userProfile.cleanlinessLevel === targetProfile.cleanlinessLevel ||
    ...
    if (userProfile.socialLevel === targetProfile.socialLevel ||
    ...
    if (commonInterests.length > 0) {
      const allInterests = [...new Set([...userProfile.interests, ...targetProfile.interests])];
      const interestScore = (commonInterests.length / allInterests.length) * 100;
    ...
    return {
      ...
      breakdown: {
        ...
        commonInterests,
        lifestyleMatch: {
          smoking: userProfile.smokingPreference === targetProfile.smokingPreference,
          pets: userProfile.petPreference === targetProfile.petPreference,
          cleanliness: userProfile.cleanlinessLevel === targetProfile.cleanlinessLevel,
  ...
  async getRecommendations(userId: string, limit = 10) {
    ...
    const profilesWithScores = await Promise.all(
      allProfiles.map(async (profile) => {
        const compatibility = await this.getCompatibilityScore(userId, profile.id);
        return {
          ...profile,
